...
@interface DetailViewContentView ()<UICollectionViewDelegate,UICollectionViewDataSource>

@property (nonatomic, weak) UIViewController *parentVC;//父视图
@property (nonatomic, strong) NSArray *childsVCs;//子视图数组
@property (nonatomic, weak) UICollectionView *collectionView;
@property (nonatomic, assign) CGFloat startOfetX;
@property (nonatomic, assign) BOOL isSelectBtn;//是否是滑动

@end

@implementation DetailViewContentView
...
{
    _isSelectBtn = NO;
    _startOfetX = scrollView.contentOffset.x;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    CGFloat currentOfetX = scrollView.contentOffset.x;
    NSInteger startIndex = floor(_startOfetX/scrollView_W);
    NSInteger endIndex;
    CGFloat progress;
    if (currentOfetX > _startOfetX) {//左滑left
        progress = (currentOfetX - _startOfetX)/scrollView_W;
        endIndex = startIndex + 1;
        if (endIndex > self.childsVCs.count - 1) {
            endIndex = self.childsVCs.count - 1;
        }
    }else if (currentOfetX == _startOfetX){//没滑过去
        progress = 0;
        endIndex = startIndex;
    }else{//右滑right
        progress = (_startOfetX - currentOfetX)/scrollView_W;
        endIndex = startIndex - 1;
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    CGFloat currentOfetX = scrollView.contentOffset.x;
    NSInteger startIndex = floor(_startOfetX/scrollView_W);
    NSInteger endIndex = floor(currentOfetX/scrollView_W);
